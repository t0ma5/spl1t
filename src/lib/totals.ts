import type { ExpenseListItem, RecurrenceRule } from '@/lib/kv/types'
...
/**
 * Aggregates total spending by category. Reimbursements are excluded, mirroring
 * the other spending totals. Categories with a net total of zero are dropped
 * and the result is sorted from highest to lowest spend.
 */
...
/**
 * Returns the individual expenses that fall within a single `YYYY-MM` month.
 */
 ...
 * paid for, and what their share of the group's expenses is in a single pass
 * over the expenses. Sorted by amount paid, descending.
 */
export function getSpendingByParticipant(
  ...
  for (const expense of expenses) {
    ...
    for (const paidBy of expense.paidBy) {
      const payer = totals.get(paidBy.id)
      if (!payer) continue
      payer.paid += paidBy.amount
      if (!counted.has(paidBy.id)) {
        payer.paidCount += 1
        counted.add(paidBy.id)
      }
    }

    for (const [participantId, share] of Array.from(
      getExpenseShares(toShareInput(expense)).entries(),
    )) {
...
/**
 * Computes high-level summary metrics over the non-reimbursement expenses.
 */
...
/**
 * Summarizes the active recurring expenses of a group. Each active recurring
 * expense is counted once per period and normalized into an estimated monthly
 * and yearly cost. Monthly and yearly are computed independently (not monthly
 * × 12) to avoid compounding the monthly rounding.
 */
