...
type SplitMode = ExpenseListItem['splitMode']
...
function makeExpense(partial: Partial<ExpenseListItem>): ExpenseListItem {
  return {
    id: 'expense',
    title: 'Expense',
    amount: 0,
    ...
    paidBy: [{ id: 'alice', name: 'Alice', amount: 0 }],
    paidFor: [],
    recurrenceRule: null,
    _count: { documents: 0 },
    originalAmount: null,
...
function splitExpense(
  ...
  return makeExpense({
    ...
    amount,
    splitMode,
    paidBy: [{ id: paidFor[0][0], name: paidFor[0][0], amount }],
...
describe('calculateShare', () => {
  ...
  it('ignores reimbursements, which balances deliberately count', () => {
    const settlement = makeExpense({
      ...
      paidBy: [{ id: 'alice', name: 'Alice', amount: 500 }],
...
describe('getSpendingByParticipant', () => {
  it('computes paid and share per participant, sorted by amount paid', () => {
    ...
    const expenses = [
      makeExpense({
        amount: 1000,
        paidBy: [{ id: 'alice', name: 'Alice', amount: 1000 }],
        paidFor: [
